 ...
 * modifications made by Cride5 improved performance by reducing number of calls to glMaterial
 */
public class DisplayListRenderer implements iModel3DRenderer{
	...
	private int pickerRenderList = -1;
	...
	public void render(Object context, Model model){
		...
		if(model.isRenderPicker()){
			...
			gl.glCallList(pickerRenderList);
		}else{
	...
	private int initialize(GL gl, Model model){
		...
		pickerRenderList = compiledList + 3;

		if(this.isDebugging){
			System.out.println("        Picker Render List");
		}
		gl.glNewList(pickerRenderList, GL.GL_COMPILE);
...
//System.out.println("verts:" + tempObj.numOfVerts + " norms:" + tempObj.normals.length);
//System.out.println("setting material " + tempObj.faces[j].materialID);
					matId = tempObj.faces[j].materialID == -1 ?
						tempObj.materialID : tempObj.faces[j].materialID;
					if(matId >= 0 && matId < model.getNumberOfMaterials()
						// Only change material if different from previously set material
							  && matId != prevMaterial
					){
						float[] rgba = new float[4];
						Material material = model.getMaterial(matId);
				...
				int normalIndex = 0;
				int textureIndex = 0;
				// Increased accuracy, worse performance
//				gl.glBegin(GL.GL_POLYGON);
				// Reduced accuracy, better performance
